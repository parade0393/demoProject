 ...
 *description：
 */
public class LetterTreeDecoration extends RecyclerView.ItemDecoration {

    private List<TreeItem> modelList;
    ...
    public LetterTreeDecoration(Context context) {
    ...
    //数据集集合
    public void setDatas(List<TreeItem> mBeans, String tagsStr) {
...
//        c.drawCircle(QMUIDisplayHelper.dp2px(mContext, 42), bottom - dividerHeight / 2, 35, mPaint);
    ...
    private boolean isSectionHeader(int position){
        if (position == 0){
            ...
            if (!modelList.get(position).getFirstLetter().equals(modelList.get(position - 1).getFirstLetter())){
                return true;
            }else {
                return false;
            }
